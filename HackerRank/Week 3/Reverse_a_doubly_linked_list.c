...
DoublyLinkedListNode* reverse(DoublyLinkedListNode* llist) {
    ...
    while (current != NULL) {
        ...
        current->next = temp; // swap next with prev
        current = current->prev; // move to the "next" node (which is old prev)
    }
    // After loop, temp will be at the node before NULL that means temp->prev is the new head
    if (temp != NULL) {A
        llist = temp->prev;
    }
    return llist;
